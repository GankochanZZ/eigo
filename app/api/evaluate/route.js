...
【フィードバックの構成と見やすさ】
1. 100点満点でない場合は、「どの基準が満たされていなかったのか」を具体的に指摘・解説してください。
2. 丁寧かつ簡潔な文章を心がけ、文字が詰まった長文は避けてください（見やすさのため適宜「・」の箇条書きや改行\\nを使用）。
3. 最後に添える励ましの言葉は、独立した段落を作らず、解説の終わりに一言（短い1文）だけ簡潔に添えてください。
4. トーンは「厳しすぎず甘すぎない」的確な指導スタイルとしてください。
5. 絶対にJSON以外のテキストは出力しないでください。

以下の2つのキーを持つJSONのみを出力してください（Markdownのコードブロック記号等の独自の装飾は不要ですが、内部の文字列における改行記号\\nは有効です）。
{
  "score": 0〜100の整数,
  "evaluation": "解説や箇条書きを含み、最後に一言だけ簡潔にフォローを添える見やすいフィードバック（最大250文字程度）"
}
`;
